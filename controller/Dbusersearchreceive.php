    ...
    try {
        ...
        while($row = mysqli_fetch_assoc($sql)){
            if($row['user_id']!=$id){
                ...
                $array_data[] = array("user_id" => $row['user_id'], "name" => $row['firstname']." ".$row['lastname'], "department" => $department, "id" => $user_id, "phonenumber" => $row['phonenumber'], "address" => $row['address'], "usertype" => ucfirst($row['usertype']), "complete_address" => $row['complete_address']);
            }
        }
