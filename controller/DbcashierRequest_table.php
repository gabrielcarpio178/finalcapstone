...
if(isset($_POST['content'])&&isset($_POST['num_page'])){
    ...
    if($content=='non_bago_table'){
        ...
        try {
            $num_sql = mysqli_query($connect, "SELECT user_tb.firstname, user_tb.lastname, student_tb.studentID_number, student_tb.year, digitalpayment_tb.payment_amount, digitalpayment_tb.payment_type, digitalpayment_tb.payment_date, digitalpayment_tb.digitalPayment_id FROM user_tb INNER JOIN digitalpayment_tb ON user_tb.user_id = digitalpayment_tb.user_id INNER JOIN student_tb ON user_tb.user_id = student_tb.user_id WHERE digitalpayment_tb.payment_type != 'Non Bago Fee'  AND requestType = 'pending' AND  digitalpayment_tb.payment_type != 'Transcript of Record';");
            $num_row = ceil(mysqli_num_rows($num_sql)/5);
        } catch (\Throwable $th) {
            echo $th;
        }
        ...
        array_push($array, $payment_date, $name, $studentID_number, $year, $payment_amount, $digitalPayment_id, $payment_ref, $num_row);
        if(count($array[5])!=0){
        ?>
            <div class="table-div mt-3">
                <table class="table table-hover text-center">
                    ...
                    for($i = 0; $i<count($array[5]); $i++){
                        ...
                        <tr id="<?=$array[6][$i] ?>">
                            ...
                            <td><?=$array[1][$i] ?></td>
                            <td><?=$array[4][$i].".00" ?></td>
                            <td><button class="btn btn-primary" onclick="accept(<?=$array[5][$i] ?>, false)">Accept</button></td>
                            <td><button class="btn btn-danger" onclick="deletePayment(<?=$array[5][$i] ?>, false)">Cancel</button></td>
                        </tr>
                    <?php
                    }
                ?>                     
            ...
            <ul class="pagination">
                <li class="page-item" >
                    <a class="page-link" href="javascript:void(0)" <?php if($num_page == 0){ echo "disabled"; }else{ ?> onclick = "displayTable('tor', <?=$num_page-1 ?>);" <?php } ?>>&laquo;</a>
                </li>
                <?php
            for($x = 1; $x<=$array[7]; $x++){
                ?>
                <li class="page-item <?=($x==$num_page)? "active": "" ?>">
                    <a class="page-link" href="javascript:void(0)" onclick="displayTable('tor', <?=$x ?>);"><?=$x ?></a>
                </li>
                <?php
            }
                ?>
                <li class="page-item">
                    <a class="page-link" href="javascript:void(0)" <?php if($array[7] == $num_page){ echo "disabled"; }else{ ?> onclick = "displayTable('tor', <?=$num_page+1 ?>);" <?php } ?>>&raquo;</a>
                </li>
            </ul>
            <?php
        }else{
