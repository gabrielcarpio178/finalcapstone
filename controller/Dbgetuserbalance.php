...
if(isset($_POST['user_id'])){
    ...
    try {
        $sql_totalPurchase =  mysqli_query($connect, "SELECT SUM(order_tb.order_amount) AS total_purchase, order_tb.user_id FROM order_tb WHERE order_tb.user_id = '$user_id' AND (order_tb.statues = 'PROCEED' OR order_tb.statues = 'ACCEPTED' OR order_tb.statues = 'PURCHASE' OR (order_tb.statues IS NULL AND CAST(order_tb.order_time AS DATE) = CAST(NOW() AS DATE)));");
